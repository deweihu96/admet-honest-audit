"""GENERIC final test evaluation (regression) -- the only file reading test for a
regression loop. Invoked once, after the loop locks its final model.

    uv run python final_test_eval_reg.py --task data_split_caco --version caco_v02_desc_lgbm

Reports: validation MAE headline, test point + bootstrap CI, valid-vs-test gap
(flagged; for MAE, test<valid is the leakage-suspicious direction), leakage
self-audit (NN Tanimoto r=3 + desalted connectivity InChIKey).
"""
import argparse
import importlib
import numpy as np
...
def main(task, version):
    ...
    print(f"\nHEADLINE validation MAE: {vs.mean():.3f} +/- {ci_hw(vs):.3f}  (primary estimate)")
    print(f"test read (single shot): point {point:.3f}, bootstrap 95% CI "
          f"[{blo:.3f}, {bhi:.3f}]  (honest test error bar)")
    print(f"test seed CI (STABILITY): +/-{ci_hw(ts):.3f}")
